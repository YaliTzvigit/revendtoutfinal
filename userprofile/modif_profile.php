...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($new_name) || empty($new_email)) {
        ...
        $update_query = $conn->prepare("UPDATE users SET namep = ?, email = ? WHERE iduser = ?");
        if (!$update_query) {
            die("Erreur dans la préparation de la requête : " . $conn->error);
        }
...
<body>
    ...
    <main>
        <form action="modif_profile.php" method="POST">
            ...
            <input type="text" id="namep" name="namep" value="<?= htmlspecialchars($user['namep']) ?>" required>
